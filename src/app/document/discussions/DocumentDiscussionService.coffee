...
###
documentDiscussionService = angular.module(
  "LivingDocuments.document.discussion.service", []
)

class DocumentDiscussionService extends BaseService
  ...
  createDiscussion: (parentId, title, selection) =>
    return @$http.post(
      ...
      @$log.debug "Created disussion with id=" + success.discussion.id
      return

DocumentDiscussionService.$inject = ['SecurityService','$http', '$log']
documentDiscussionService.service(
